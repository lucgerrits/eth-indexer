...
pub async fn insert_transaction_receipt(
    ...
    let status = if transaction_receipt.status.unwrap_or_default().as_u32() == 1 { true } else { false };
    let effective_gas_price = Decimal::from(transaction_receipt.effective_gas_price.unwrap_or_default().as_u128());
